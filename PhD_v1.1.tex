...
\end{equation}
where $\alpha$ is an absorption coefficient of a solid. The absorption coefficient is closely related to the quantum-mechanical transition rate $W_{if}$ from the initial state $i$ to the final state $f$ is given by the Fermi Golden rule. This rate can be expressed by
\begin{equation}
W_{if} = \frac{2\pi}{\hbar} \rho (E) |\mathcal{H}_{if}'|^2.
\label{eq:FermiGolderRule}
\end{equation}
In this expression, the matrix element $\mathcal{H}_{if}'$ corresponds to the optical external perturbation on the electrons a $\rho (E)$ is the density of states (DOS) function describing the initial and final states with energy difference $E$, associated with an energy of the excitation photon ($E = \hbar \omega$). It is easy to see that the Hamiltonian $\mathcal{H}_{if}'$ can be interpreted as a optical perturbation. A standard one-electron Hamiltonian without magnetic field takes form of:
\begin{equation}
...
\myparagraph{DOS in parabolic band}
\wciecie

The DOS function $\rho$ from Equation \ref{eq:L-B_law} is energy-dependent it has to be calculated at the energy $E = \hbar \omega$ of the excitation photons, since the initial and final states are separated in energy by $E$. Because of that, the function $\rho (E)$ was named \textit{optical joint density of states}.



\begin{equation}
\label{abs_coeff2}
\alpha (\omega) = A \sum_{i,f} P_{if} n_i n_f,
\end{equation}

\begin{equation}
\label{abs_coeff}
\alpha (\omega) = \frac{1}{2\pi^2} \left(\frac{2m^*}{\hbar^2} \right)^{(3/2)} (\hbar\omega-E_g)^{(1/2)} ,
\end{equation}


\section{Interband and intraband transitions}
