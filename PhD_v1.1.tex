...
\label{eq:DOS_6}
\begin{aligned}
dN_{3D} = \rho_D (k) dV_k = \frac{2}{(2\pi)^3} \cdot 4\pi k^2 dk,\\
dN_{2D} = \rho_D (k) dV_k = \frac{2}{(2\pi)^2} \cdot 2\pi k dk.
\end{aligned}
\end{eqnarray}

Substituting Equations \ref{eq:DOS_5} and \ref{eq:DOS_6} into Equation \ref{eq:DOS_4} gives the DOS function per unit energy for parabolic bands 
\begin{eqnarray}
\label{eq:DOS_7}
\begin{aligned}
\rho_{3D}(E) = \frac{1}{2\pi^2} \left(\frac{2m^*}{\hbar^2} \right)^{(3/2)} (E)^{(1/2)},\\
\rho_{2D}(E) = \frac{m^*}{\pi\hbar^2}, 
\end{aligned}
\end{eqnarray}


The DOS function $\rho$ from Equation \ref{eq:DOS_2} is energy-dependent it has to be calculated at the energy $E = \hbar \omega$ of the excitation photons, since the initial and final states are separated in energy by $E$. Because of that, the function $\rho (E)$ was named \textit{optical joint density of states}.
